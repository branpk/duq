...
def op_map(ctx: DuqContext, input: dict[str, Any], body: Expr) -> dict[str, Any]:
    return {key: ctx.evaluate(body, value) for key, value in input.items()}
...
def op_mapField(
    ...
    result = dict(input)
    result[field_name] = ctx.evaluate(body, input[field_name])
    return result
