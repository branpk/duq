...
def get_op_signature(name: str, op_defn: Callable) -> OpSignature:
    name_parts = name.split(".")
    if len(name_parts) != 3:
        raise Exception(
            f"op name must have form `<package>.<namespace>.<op>`: `{name}`"
        )

    signature = inspect.signature(op_defn)
    ...
    for i, param in enumerate(signature.parameters.values()):
        if param.name == "input":
            if i != 0:
                raise Exception("`input` must be the first parameter")
            if param.kind != Parameter.POSITIONAL_OR_KEYWORD:
                raise Exception(f"invalid input parameter: `{param}`")
            input_param = param
            ...
            raise Exception(f"invalid op parameter: {param}")
    ...
    if is_macro and not all_expr_args:
        raise Exception(f"mixture of Expr and concrete op parameters")
...
def type_check_op_call(
    signature: OpSignature, input: Any, args: tuple[Any, ...]
) -> None:
    if signature.input_param:
        type_check_value(input, signature.input_param.annotation)

    req_params = signature.req_arg_params
    opt_params = signature.opt_arg_params
    var_param = signature.var_args_param
